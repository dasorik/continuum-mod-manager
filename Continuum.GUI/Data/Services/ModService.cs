...
namespace Continuum.GUI.Services
{
	...
	public class ModService
	{
...
#else
		const string TOOL_PATH = "resources\\tools";
#endif
		...
		{
#if DEBUG
			string executionPath = new DirectoryInfo(Path.Combine(Global.CONTENT_ROOT, "..")).FullName;
#else
			string executionPath = new DirectoryInfo(Path.Combine(Global.CONTENT_ROOT, "..\\..\\..")).FullName;
#endif
			...
			this.integrationFolder = Path.Combine(Global.APP_DATA_FOLDER, "Integrations");
			this.modFolder = Path.Combine(Global.APP_DATA_FOLDER, "Mods");
		...
		{
			Logger.Log($"Reloading Integrations from folder '{integrationFolder}'", LogSeverity.Info);

			var version = Assembly.GetExecutingAssembly().GetName().Version;
			var versionString = $"{version.Major}.{version.Minor}";

			var loader = new GameIntegrationLoader(integrationCacheFolder, versionString, "1.0"); // Hardcoded for the time being, to ensure that integrations are supported after major version release
			var integrationPaths = Directory.GetFiles(integrationFolder);
		...
		public void ReloadMods()
		{
			...
			var loader = new ModLoader(modCacheFolder, AvailableIntegrations);
			var modPaths = Directory.GetFiles(modFolder);
		...
		public LoadResult<GameIntegration> TryAddIntegration(string fileName, byte[] fileBytes)
		{
			var installPath = Path.Combine(integrationFolder, fileName);

			if (File.Exists(installPath))
				installPath = FileWriter.GetUniqueFilePath(installPath);

			var fileInfo = new FileInfo(installPath);
			File.WriteAllBytes(installPath, fileBytes);
		...
		public LoadResult<ModConfiguration> TryAddMod(string fileName, byte[] fileBytes)
		{
		...
		public void DeleteMod(ModConfiguration mod)
		{
			...
			File.Delete(Path.Combine(modFolder, loadedMod.fileName));
		...
		public void DeleteIntegration(GameIntegration integration)
		{
			...
			File.Delete(Path.Combine(integrationFolder, integrationData.fileName));
